...
hb9xar, 25-Nov-2006: BC conflicts wth termcap.h. changed to BC_
...
void term_exit()
{
  if (termcap) {
    ...
    free(cbuf);
     ...
     free(WR); free(CL);
     free(IS); free(BC_); free(ND); free(CE);
...
int extcols;
{
 ...
 if (termcap) {
  ...
  if ((NL = tgetstr("nl", &_tptr)) == NULL)
    NL = "\012";
  if (tgetflag("bs") || ((BC_ = tgetstr("bc", &_tptr)) == NULL))
    BC_ = "\010";
