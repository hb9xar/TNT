...
extern long int session_sek;
extern char tnt_info_message[80];
                   ...
                   time_t start_time)
{
  ...
  while((c = getbyte(in)) != EOF) {
	...
	case 'a':
		...
		hour = (diffmin / 60);
                diffmin %= 60;
		if(hour>0) sprintf(namestr,"%dh %dm",hour,diffmin);
        else {
		  sprintf(namestr,"%d Minute",diffmin);
        ...
        case 'L':
          ...
          hour = (diffmin / 60);
          diffmin %= 60;
          sprintf(namestr,"%2.2u:%2.2u",hour,diffmin);
          putstring(namestr,out);
          break;
	...
	case 's':
		diffmin = ((time(NULL) - tnt_startup ) + session_sek) / 60;
                ...
                hour = (diffmin / 60) % 60;
                diffmin %= 60;
		if(day>0) sprintf(namestr,"%dd %dh %dm",day,hour,diffmin);
		else sprintf(namestr,"%dh %dm",hour,diffmin);
		putstring(namestr,out);
		break;
