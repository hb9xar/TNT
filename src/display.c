/* tnt: Hostmode Terminal for TNC
   ...
   updated: Matthias Hensler WS1LS 99/02/05
  ...
  for (i = (0+ch_offset); i < (10+ch_offset); i++) {
    if (i >= tnc_channels) {
      ...
      if (ch_stat[i].not_disp) {
        ...
        strip_call_log(call_sign, i);
        for(tmp=0; tmp<strlen(call_sign); tmp++) {
          ch_str[0] = tolower(call_sign[tmp]);
    ...
    for(tmp=0; tmp<strlen(tmp_call); tmp++) {
      if(tmp_call[tmp] == '-') tmp_call[tmp] = '\0';
...
char *buffer;
{
  ...
  if(decstathuf(buffer, decomp_buffer)==0)
  {
    sprintf(comp_info, ">>> Hufmann-compressed (%0.1f %%)",
