...
char debugiof[80];
#endif
char tnt_telltext_file[80];
#ifdef TNT_SOUND
char tnt_sound_file[80];
char sound_dir[80];
#endif
char tnt_session_log[80];
char tnt_qtextfile[80];
#ifdef BCAST
...
char *str2;
{
  ...
  else if (strcmp(str1, "sound_dir") == 0) {
    strcpy(sound_dir, str2);
    return(0);
  }
#endif
...
static int update_filenames()
{
  ...
  add_tntdir(tnt_telltext_file);
#ifdef TNT_SOUND
  add_tntdir(tnt_sound_file);
#endif
#endif
...
char *str;
{
  ...
  cmd_display(mode,channel,sound_dir,1);
#endif
...
int *unlock;
{
  ...
  strcpy(sound_dir, DEF_SOUND_DIR);
#endif
...
/* WSPse: Try to find a tnt configuration file (22.2.1999) */
  if (explized_ini == 1) {   /* Explizites File angegeben */
    if (!(init_file_fp = fopen(tnt_initfile,"r"))) {
      printf("ERROR: explizit configuration \"%s\" not found.\n\n",
              tnt_initfile);
      return(1);
    }
