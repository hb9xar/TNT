...
extern void rem_mb_display_buf(char *buffer,int len);
    ...
    * so here is no ifdef around , dg9ep
    */
 static char lastGPGPEDITfilename[257]; /* dg9ep */

extern void reinit_screen();
extern int rem_datei_display();  /* dg9ep */
extern int analyse_remstr(int channel,char *remstring,int autost); /* DH3MB */
...
int cscript;
{
  ...
  if(mode != M_EXTMON && mode != M_MAILBOX &&
     strstr(str,"//") == str) {
    char temp[160];
    sprintf(temp, "Sysop forced execution of remote command %s...", str);
...
char *str;
{
  ...
  timestr = *localtime(&timeval);
  sprintf(tmpstr,"K %2.2u.%2.2u.%2.2u",
          timestr.tm_mday,timestr.tm_mon+1,timestr.tm_year);
  queue_cmd_data(channel,X_COMM,strlen(tmpstr),mode,tmpstr);
  cmd_display(mode,channel,tmpstr,1);
  tnc_command = 1;
}
...
{ char tx_buf[2];
  ...
  if(tx_buf[0]>='@' && tx_buf[0]<='Z') {
    ...
    cmd_display(0,channel,
                "No corresponding control-character available",1);
}
...
char *str;
{
  ...
  /* sprintf(cmdline,"par1:%d par2:%d chan:%d len:%d mode:%d str:%s",par1,par2,channel,len,mode,str);
   * cmd_display(mode,channel,cmdline,1);
   * return;
   */
  ...
  if( !str || (strlen(str) == 0) ) {
    cmd_display(mode,channel,"pgp password missing",1);
    return;
  }
  ...
  if( !sEditor || (strlen(sEditor) == 0) )   /* default to ... */
    sEditor = "/usr/bin/editor"; /* debian-policy 4.3 */
  /* look for Editorbinary */
  if (stat(sEditor,&fileinfo) != 0) {
      char temp[160];
      sprintf(temp, "Sorry, Editor ""%s"" not found/not executable, check env.var EDITOR ", sEditor);
      cmd_display(mode,channel,temp,1);
      return;
  }
...
cmd_display(mode,channel,cmdline,1);
  ...
  /* does tmpeditname exist? */
  if (!(hdl_test = fopen(tmpeditname,"r"))) {
    /* Keine Arme, keine Kekse */
    cmd_display(mode,channel,"No edited file -> no signing, sigh...",1);
    return;
  }
  fclose(hdl_test);
  ...
  if( par1==GPGP_PGP ) {
     ...
     mkstemp(tmppwfilename);
     if( !(pw = fopen(tmppwfilename,"w+")) ) {
       cmd_display(mode,channel,"*** error: can't write tmpfile",1);
       return;
     }
     fprintf(pw,"%s",str);
     fclose(pw);
     ...
     sprintf(line ,"\r*** calling GnuPG *****\r");
  }
  ...
  /* sprintf(line,"\rres pgp=%d\r%s\r ",result,cmdline);
   * rem_data_display(channel,line);
   */
  ...
  if(par1 == GPGP_GPG) {
     if( !(pw = fopen(tmppwfilename,"r+")) ) {
       ...
       cmd_display(mode,channel,"*** error: can't rewrite tmpfile",1);
       return;
     }
