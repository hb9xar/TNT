...
int sel_remhlist(int fd1, int flag, char *rem_newlin_str)
{
  ...
  if(flag == 0) {
    if(cou = 0) strcpy(heardstring, "[Keine Eintraege vorhanden]");
    else        strcpy(heardstring, "[Alle Eintraege gelistet]");
    strcat(heardstring, rem_newlin_str);
    write(fd1, heardstring, strlen(heardstring));
  }
  return(cou);
}
...
/* Den Sinn der Funktion get_qrg_from_chanstr() kann ich leider nicht nach-
   vollziehen.
   Ich kann aber sehr voll nachvollziehen, warum TNT an dieser Stelle mit
   einem Segmentation Fault rausknallt. Unser Char-Array "chanstr" ist auf
   11 Bytes begrenzt. Passiert es aber, daß im Buffer keine QRG auftaucht,
   wird diese 11 Byte-Grenze bei weitem überschritten. Ein ":" kommt in
   sowieso immer im buffer vor, nämlich wenn die Timestamps eingeschaltet
   sind. In diesem Fall werden über 40 Bytes in "chanstr" kopiert, was nicht
   gut geht.
   Bis jemand eine bessere Lösung findet, sollte dieser Workaround helfen.
   Durch die Modulo-Funktion verhindern wir jegliches Kopieren über diese
   Grenze hinaus.
   Matthias Hensler - 4. Feb. 1999 */

  if (ptr != NULL) {
    strncpy(chanstr,buffer,(ptr+1-buffer)%10);
    chanstr[(ptr+1-buffer)%10] = '\0';
  }
